  ...
  try {
    ...
    await pool.query(`
      ...
      BEGIN 
        ...
        IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE table_name='s_categories' AND constraint_name='s_categories_name_key') THEN
          ...
          -- Add new unique constraint on (account_id, name) if it doesn't exist
    ...
    for (const table of tablesToUpdate) {
      if (table !== 's_users') {
        ...
        // For users, only update those that aren't super_admin
...
async function setupApp() {
  ...
  app.post("/api/categories", authenticateToken, async (req, res) => {
    const { name } = req.body;
    try {
      // Use IS NOT DISTINCT FROM for NULL-safe equality check
  ...
  app.delete("/api/categories/:id", authenticateToken, async (req, res) => {
    ...
    try {
      ...
      if (req.user.role !== 'super_admin') {
        // Admins can only delete categories belonging to their account
        query += " AND account_id = $2";
        params.push(req.user.account_id);
      }
      const result = await pool.query(query, params);
      console.log(`DELETE /api/categories/${id} - rows updated: ${result.rowCount} for user ${req.user.email} (role: ${req.user.role}, account: ${req.user.account_id})`);
      res.sendStatus(204);
    } catch (err) {
  ...
  app.post("/api/users", authenticateToken, async (req, res) => {
    ...
    // Only super_admin can create users in any account or create other super_admins
    // Admins can only create users in their own account
    if (req.user.role !== 'super_admin') {
  ...
  app.delete("/api/users/:id", authenticateToken, async (req, res) => {
    const { id } = req.params;
    try {
      // Check if user exists and belongs to the same account (unless super_admin)
      const userCheck = await pool.query("SELECT account_id, role FROM s_users WHERE id = $1", [id]);
      if (userCheck.rows.length === 0) return res.status(404).json({ message: "User not found" });
